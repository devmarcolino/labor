...
<body class="bg-gray-50 dark:bg-gray-900 transition-colors duration-500 min-h-screen">
  ...
  <header class="p-2 flex justify-between w-full"> 
    ...
    <div class="shadow-sm hover:shadow-md ease-in bg-gray-50/15 dark:bg-gray-800 dark:border-gray-600 flex items-center justify-between gap-3 w-full border border-gray-300 rounded-full px-5 py-5 mt-2 transition-all">
      ...
      <div class="flex gap-1">
        @for($i = 1; $i <= 5; $i++)
            @if($i <= round($mediaNota))
                <!-- ESTRELA CHEIA (Amarela) -->
                <svg class="w-6 h-6 text-yellow-400" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                </svg>
            @else
                <!-- ESTRELA VAZIA (Cinza/Outline) -->
                <svg class="w-6 h-6 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      ...
      <div class="bg-white dark:bg-gray-800 items-center justify-center text-center rounded-[40px] shadow-labor w-full max-w-2xl py-3 px-5">
        ...
        <!-- Botão MEU ENDEREÇO -->
        <x-btn-outline-account href="{{ route('workers.edit.address') }}">
