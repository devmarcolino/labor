...
<body class="bg-white dark:bg-gray-900 transition-colors duration-500 h-screen flex flex-col overflow-hidden">
    ...
    <header class="flex items-center justify-between py-3 px-5 fixed top-0 left-0 w-full z-10 bg-white shadow-sm"
        ...
        <ul class="flex items-center py-1.5 px-2.5 bg-gray-100 dark:bg-gray-800 rounded-full shadow-sm"
            ...
            <li role="presentation">
                <a href="{{ url('workers/dashboard') }}"
                    ...
                    id="flame-tab" data-tabs-target="#flame" type="button" role="tab" aria-controls="flame"
                    aria-selected="false">
                ...
                <a href="{{ url('workers/dashboard') }}"
                    ...
                    <div class="hidden group-aria-selected:block">
                        <svg class="w-6 h-6" viewBox="0 0 24 24" fill="#0284C7">
                            <path
                                d="M11.47 3.841a.75.75 0 0 1 1.06 0l8.69 8.69a.75.75 0 1 0 1.06-1.06l-8.689-8.69a2.25 2.25 0 0 0-3.182 0l-8.69 8.69a.75.75 0 1 0 1.06 1.06l8.69-8.69Z" />
                            <path
                                d="M12 5.432 4.5 12.932V21a1.5 1.5 0 0 0 1.5 1.5h4.5V18A1.5 1.5 0 0 1 12 16.5h.008A1.5 1.5 0 0 1 13.5 18v4.5h4.5A1.5 1.5 0 0 0 19.5 21V12.932L12 5.432Z" />
                        </svg>
                    </div>
                </a>
            </li>
        ...
        <a href="{{ url('workers/account') }}"
            class="text-gray-600 hover:text-gray-900 shadow-labor">
            @php
                $workerAvatar = isset($worker)
                    ? ($worker->fotoUser ?? null)
                    : (Auth::user()->fotoUser ?? null);
            @endphp
            <div class="bg-gray-200 bg-center bg-cover bg-no-repeat w-[40px] h-[40px] rounded-full shadow-md"
                style="background-image: url('{{ $workerAvatar ? asset('storage/' . $workerAvatar) : asset('img/default-avatar.png') }}');">
            </div>
        </a>
    </header>
    ...
    @php
        ...
        $workerId = Auth::id();
        $empresasIds = Mensagem::where(function($q) use ($workerId) {
            $q->where('remetente_id', $workerId)->where('remetente_tipo', 'user');
        })->orWhere(function($q) use ($workerId) {
            $q->where('destinatario_id', $workerId)->where('destinatario_tipo', 'user');
        })
        ->where(function($q) {
            $q->where('remetente_tipo', 'empresa')->orWhere('destinatario_tipo', 'empresa');
        })
        ->get()
        ->map(function($msg) use ($workerId) {
            return $msg->remetente_tipo === 'empresa' ? $msg->remetente_id : $msg->destinatario_id;
        })
        ->unique();
    ...
    @if($cardsEmpresas->count() > 0)
        <div class="w-full flex flex-col gap-2 py-3 px-4 overflow-y-auto">
    ...
    @else
        <div class="w-full flex justify-center items-center bg-gray-100/80 rounded-full py-3 mt-5">
